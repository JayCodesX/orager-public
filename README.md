...
orager is a TypeScript library and CLI for building AI agents with persistent memory, self-improving skills, and multi-model routing. Agents remember context across sessions, capture successful patterns into reusable skills, and optionally train LoRA adapters from their own trajectory data — all running locally or in the cloud.
...
| **Confidence routing** | 3-signal router: task classifier, Self-REF, semantic entropy |
...
| `orager skill-train` | OMLS training (LoRA adapters) |
...
| `orager optimize` | Self-optimize agent prompts (GEPA) |
| `orager benchmark` | Benchmark agents on tasks |
...
- **Orager Cloud** — managed agents, hosted memory, zero infrastructure
- **SkillBank Pro** — shared skills across teams + managed OMLS training jobs
