...
OMLS training features (LoRA adapters, confidence router, prompt tournaments, meta-optimizer) require a **Pro** or **Cloud** license. SkillBank prompt-mode extraction is available on all tiers. See [Licensing & Tiers](/guide/licensing) for details.
:::
...
### Prompt Mode (SkillBank-Only) — All Tiers
...
- **License**: Available on all tiers including Open Source.

### LoRA Mode (Weight Training) — Pro / Cloud
